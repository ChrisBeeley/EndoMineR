...
#cbind(EndoscMeds(Myendo$Medications), Myendo)
#pander(head(cbind(EndoscMeds(Myendo$Medications), Myendo)[1:4],5))
...
Of course, the data disualisation part of this package (templated to produce publication ready graphs amongst other things) can be used with this data in one line as follows:
```{r exampleHistolTypeAndSite, echo = FALSE}

Myendo2<-Endomerge2(Myendo,'Dateofprocedure','HospitalNumber',Mypath,'Dateofprocedure','HospitalNumber')
PathSiteAndType <- HistolTypeAndSite(Myendo2$PathReportWhole,Myendo2$Macroscopicdescription, Myendo2$ProcedurePerformed)
pander(head(data.frame(PathSiteAndType),5))
